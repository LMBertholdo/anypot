...
class DB_DNSTor:
    """
    NTP database and clean memory struct
    """
    ...
    def _run_db(self):
        try:
            ...
            print('[ntp _run_db function]',e)
            self.my_logger.critical('[ntp _run_db function]' + str(e))
    ...
    def _create_tables(self):
        try:
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS NTP_BASIC_CONFIG
                (
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS NTP_MEMORY_DICT
                (
                     ip   text    NOT NULL
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS NTP_IGNORE_IP_DICT
                (
                     ip   text    NOT NULL
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS NTP_PAYLOAD_DICT
                (
                     payloadID   integer    NOT NULL
            ...
            print('[ntp _create_tables function]',e)
            self.my_logger.critical('[ntp _create_tables function]' + str(e))

    def _insert_basic_verify(self,bind_address):
        try:
            cur = self.conn.cursor()

            cmdSelect = '''SELECT bind_address FROM NTP_BASIC_CONFIG limit 1;'''
            ...
            print('[ntp _insert_basic_verify function]',e)
            self.my_logger.critical('[ntp _insert_basic_verify function]' + str(e))
    ...
    def _insert_basic(self, bind_address, bind_port):
        try:
            ...
            print('[ntp _insert_basic function]',e)
            self.my_logger.critical('[ntp _insert_basic function]' + str(e))

    def insert_basic_config(self,bind_address, bind_port):
        try:
            cur = self.conn.cursor()
            varStore = '''INSERT OR REPLACE INTO NTP_BASIC_CONFIG (bind_address,bind_port,date_start) VALUES(:bind_address,:bind_port,:date_start);'''
            cur.execute(varStore, {'bind_address':bind_address ,'bind_port':bind_port , 'date_start':datetime.datetime.now()})
            self.conn.commit()
            cur.close()

        except Exception as e:
            print('[ntp insert_basic_config function]',e)
            self.my_logger.critical('[ntp insert_basic_config function]' + str(e))
    ...
    def insert_ignore_ip(self):
        ...
        try:
            ...
            for i in range(len(Memory.sufixo)):
                ...
                varStore = '''INSERT OR REPLACE INTO NTP_IGNORE_IP_DICT
                (ip,sufixo,ref)
            ...
            print('[ntp insert_ignore_ip function]',e)
            self.my_logger.critical('[ntp insert_ignore_ip function]' + str(e))
    ...
    def lookup_ignore(self, name):
        try:
            ...
            print('[ntp lookup_ignore function]',e)
            self.my_logger.critical('[ntp lookup_ignore function]' + str(e))
    ...
    def _clean_dict(self):
        try:
            ...
            print('[ntp _clean_dict function]',e)
            self.my_logger.critical('[ntp _clean_dict function]' + str(e))
    ...
    def _control_clean_dict(self):
        try:
            ...
            print('[ntp _control_clean_dict function]',e)
            self.my_logger.critical('[ntp _control_clean_dict function]' + str(e))
    ...
    def _verify__count_use(self):
        ...
        try:
            ...
            print('[ntp _verify__count_use function]',e)
            self.my_logger.critical('[ntp _verify__count_use function]' + str(e))

    def searchPayloads(self, payloadSearch):
        try:
            # CHANGE OLD TABLES with
            # alter table NTP_MEMORY_DICT add payloadID status integer default '-1' ;

            cur = self.conn.cursor()

            tValue = (str(payloadSearch), )
            cur.execute('''SELECT payloadID FROM NTP_PAYLOAD_DICT where payload LIKE ? limit 1;''', tValue)
            rowDB = cur.fetchone()
            # print(rowDB[0])

            if(rowDB == None):
                cmdSelect = '''SELECT payloadID FROM NTP_PAYLOAD_DICT order by payloadID DESC limit 1;'''
                ...
                if(Memory.flag_print == True):
                    print('[ntp New ID]:', lastID)

                # Insert new key and payload
                iValue = (int(lastID), str(payloadSearch), )
                cur.execute('''INSERT INTO NTP_PAYLOAD_DICT VALUES (?,?)''', iValue)
                self.conn.commit()
                ...
                cur.close()
                if(Memory.flag_print == True):
                    print('[ntp Found ID]:', rowDB[0])

                return int(rowDB[0])

        except Exception as e:
            print('[ntp searchPayloads function]',e)
            self.my_logger.critical('[ntp searchPayloads function]' + str(e))
            cur.close()
            return int(-1)
    ...
    def _insert_dict(self,ip):
        ...
        try:
            ...
            Memory.lock.acquire()
            cur = self.conn.cursor()
            varStore = '''INSERT INTO NTP_MEMORY_DICT (ip,count,tempoInicio,tempoFinal,payloadID) VALUES(:ip,:count,:tempoInicio,:tempoFinal,:payloadID);'''
            ...
            print('[ntp _insert_dict function]',e)
            self.my_logger.critical('[ntp _insert_dict function]' + str(e))
    ...
    def _verify_time_of_last_dict_use(self):
        try:
            ...
            print('[ntp _verify_time_of_last_dict_use function]',e)
            self.my_logger.critical('[ntp _verify_time_of_last_dict_use function]' + str(e))
