...
class DB_DNSTor:
    """
    cldap database and clean memory struct
    """
    ...
    def _run_db(self):
        try:
            ...
            print('[cldap _run_db function]',e)
            self.my_logger.critical('[cldap _run_db function]' + str(e))
    ...
    def _create_tables(self):
        try:
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS CLDAP_BASIC_CONFIG
                (
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS CLDAP_MEMORY_DICT
                (
                     ip   text    NOT NULL
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS CLDAP_IGNORE_IP_DICT
                (
                     ip   text    NOT NULL
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS CLDAP_PAYLOAD_DICT
                (
                     payloadID   integer    NOT NULL
            ...
            print('[cldap _create_tables function]',e)
            self.my_logger.critical('[cldap _create_tables function]' + str(e))

    def _insert_basic_verify(self,bind_address):
        try:
            cur = self.conn.cursor()

            cmdSelect = '''SELECT bind_address FROM CLDAP_BASIC_CONFIG limit 1;'''
            ...
            print('[cldap _insert_basic_verify function]',e)
            self.my_logger.critical('[cldap _insert_basic_verify function]' + str(e))
    ...
    def _insert_basic(self, bind_address, bind_port):
        try:
            ...
            print('[cldap _insert_basic function]',e)
            self.my_logger.critical('[cldap _insert_basic function]' + str(e))

    def insert_basic_config(self,bind_address, bind_port):
        try:
            cur = self.conn.cursor()
            varStore = '''INSERT OR REPLACE INTO CLDAP_BASIC_CONFIG (bind_address,bind_port,date_start) VALUES(:bind_address,:bind_port,:date_start);'''
            cur.execute(varStore, {'bind_address':bind_address ,'bind_port':bind_port , 'date_start':datetime.datetime.now()})
            self.conn.commit()
            cur.close()

        except Exception as e:
            print('[cldap insert_basic_config function]',e)
            self.my_logger.critical('[cldap insert_basic_config function]' + str(e))
    ...
    def insert_ignore_ip(self):
        ...
        try:
            ...
            for i in range(len(Memory.sufixo)):
                ...
                varStore = '''INSERT OR REPLACE INTO CLDAP_IGNORE_IP_DICT
                (ip,sufixo,ref)
            ...
            print('[cldap insert_ignore_ip function]',e)
            self.my_logger.critical('[cldap insert_ignore_ip function]' + str(e))
    ...
    def lookup_ignore(self, name):
        try:
            ...
            print('[cldap lookup_ignore function]',e)
            self.my_logger.critical('[cldap lookup_ignore function]' + str(e))
    ...
    def _clean_dict(self):
        try:
            ...
            print('[cldap _clean_dict function]',e)
            self.my_logger.critical('[cldap _clean_dict function]' + str(e))
    ...
    def _control_clean_dict(self):
        try:
            ...
            print('[cldap _control_clean_dict function]',e)
            self.my_logger.critical('[cldap _control_clean_dict function]' + str(e))
    ...
    def _verify__count_use(self):
        try:
            ...
            print('[cldap _verify__count_use function]',e)
            self.my_logger.critical('[cldap _verify__count_use function]' + str(e))

    def searchPayloads(self, payloadSearch):
        try:
            # CHANGE OLD TABLES with
            # alter table CLDAP_MEMORY_DICT add payloadID status integer default '-1' ;

            cur = self.conn.cursor()

            tValue = (str(payloadSearch), )
            cur.execute('''SELECT payloadID FROM CLDAP_PAYLOAD_DICT where payload LIKE ? limit 1;''', tValue)
            rowDB = cur.fetchone()
            # print(rowDB[0])

            if(rowDB == None):
                cmdSelect = '''SELECT payloadID FROM CLDAP_PAYLOAD_DICT order by payloadID DESC limit 1;'''
                ...
                if(Memory.flag_print == True):
                    print('[cldap New ID]:', lastID)

                # Insert new key and payload
                iValue = (int(lastID), str(payloadSearch), )
                cur.execute('''INSERT INTO CLDAP_PAYLOAD_DICT VALUES (?,?)''', iValue)
                self.conn.commit()
                ...
                cur.close()
                if(Memory.flag_print == True):
                    print('[cldap Found ID]:', rowDB[0])

                return int(rowDB[0])

        except Exception as e:
            print('[cldap searchPayloads function]',e)
            self.my_logger.critical('[cldap searchPayloads function]' + str(e))
            cur.close()
            return int(-1)
    ...
    def _insert_dict(self,ip):
        ...
        try:
            ...
            Memory.lock.acquire()
            cur = self.conn.cursor()
            varStore = '''INSERT INTO CLDAP_MEMORY_DICT (ip,count,tempoInicio,tempoFinal,payloadID) VALUES(:ip,:count,:tempoInicio,:tempoFinal,:payloadID);'''
            ...
            print('[cldap _insert_dict function]',e)
            self.my_logger.critical('[cldap _insert_dict function]' + str(e))
    ...
    def _verify_time_of_last_dict_use(self):
        try:
            ...
            print('[cldap _verify_time_of_last_dict_use function]',e)
            self.my_logger.critical('[cldap _verify_time_of_last_dict_use function]' + str(e))
