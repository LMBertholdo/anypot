...
class DB_DNSTor:
    """
    CoAP database and clean memory struct
    """
    ...
    def _run_db(self):
        try:
            ...
            print('[coap _run_db function]',e)
            self.my_logger.critical('[coap _run_db function]' + str(e))
    ...
    def _create_tables(self):
        try:
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS COAP_BASIC_CONFIG
                (
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS COAP_MEMORY_DICT
                (
                     ip   text    NOT NULL
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS COAP_IGNORE_IP_DICT
                (
                     ip   text    NOT NULL
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS COAP_PAYLOAD_DICT
                (
                     payloadID   integer    NOT NULL
            ...
            print('[coap _create_tables function]',e)
            self.my_logger.critical('[coap _create_tables function]' + str(e))

    def _insert_basic_verify(self,bind_address):
        try:
            cur = self.conn.cursor()

            cmdSelect = '''SELECT bind_address FROM COAP_BASIC_CONFIG limit 1;'''
            ...
            print('[coap _insert_basic_verify function]',e)
            self.my_logger.critical('[coap _insert_basic_verify function]' + str(e))
    ...
    def _insert_basic(self, bind_address, bind_port):
        try:
            ...
            print('[coap _insert_basic function]',e)
            self.my_logger.critical('[coap _insert_basic function]' + str(e))

    def insert_basic_config(self,bind_address, bind_port):
        try:
            cur = self.conn.cursor()
            varStore = '''INSERT OR REPLACE INTO COAP_BASIC_CONFIG (bind_address,bind_port,date_start) VALUES(:bind_address,:bind_port,:date_start);'''
            cur.execute(varStore, {'bind_address':bind_address ,'bind_port':bind_port , 'date_start':datetime.datetime.now()})
            self.conn.commit()
            cur.close()

        except Exception as e:
            print('[coap insert_basic_config function]',e)
            self.my_logger.critical('[coap insert_basic_config function]' + str(e))
    ...
    def insert_ignore_ip(self):
        ...
        try:
            ...
            for i in range(len(Memory.sufixo)):
                ...
                varStore = '''INSERT OR REPLACE INTO COAP_IGNORE_IP_DICT
                (ip,sufixo,ref)
            ...
            print('[coap insert_ignore_ip function]',e)
            self.my_logger.critical('[coap insert_ignore_ip function]' + str(e))
    ...
    def lookup_ignore(self, name):
        try:
            ...
            print('[coap lookup_ignore function]',e)
            self.my_logger.critical('[coap lookup_ignore function]' + str(e))
    ...
    def _clean_dict(self):
        try:
            ...
            print('[coap _clean_dict function]',e)
            self.my_logger.critical('[coap _clean_dict function]' + str(e))
    ...
    def _control_clean_dict(self):
        try:
            ...
            print('[coap _control_clean_dict function]',e)
            self.my_logger.critical('[coap _control_clean_dict function]' + str(e))
    ...
    def _verify__count_use(self):
        ...
        try:
            ...
            print('[coap _verify__count_use function]',e)
            self.my_logger.critical('[coap _verify__count_use function]' + str(e))

    def searchPayloads(self, payloadSearch):
        try:
            # CHANGE OLD TABLES with
            # alter table COAP_MEMORY_DICT add payloadID status integer default '-1' ;

            cur = self.conn.cursor()

            tValue = (str(payloadSearch), )
            cur.execute('''SELECT payloadID FROM COAP_PAYLOAD_DICT where payload LIKE ? limit 1;''', tValue)
            rowDB = cur.fetchone()
            # print(rowDB[0])

            if(rowDB == None):
                cmdSelect = '''SELECT payloadID FROM COAP_PAYLOAD_DICT order by payloadID DESC limit 1;'''
                ...
                if(Memory.flag_print == True):
                    print('[coap New ID]:', lastID)

                # Insert new key and payload
                iValue = (int(lastID), str(payloadSearch), )
                cur.execute('''INSERT INTO COAP_PAYLOAD_DICT VALUES (?,?)''', iValue)
                self.conn.commit()
                ...
                cur.close()
                if(Memory.flag_print == True):
                    print('[coap Found ID]:', rowDB[0])

                return int(rowDB[0])

        except Exception as e:
            print('[coap searchPayloads function]',e)
            self.my_logger.critical('[coap searchPayloads function]' + str(e))
            cur.close()
            return int(-1)
    ...
    def _insert_dict(self,ip):
        ...
        try:
            ...
            Memory.lock.acquire()
            cur = self.conn.cursor()
            varStore = '''INSERT INTO COAP_MEMORY_DICT (ip,count,tempoInicio,tempoFinal,payloadID) VALUES(:ip,:count,:tempoInicio,:tempoFinal,:payloadID);'''
            ...
            print('[coap _insert_dict function]',e)
            self.my_logger.critical('[coap _insert_dict function]' + str(e))
    ...
    def _verify_time_of_last_dict_use(self):
        try:
            ...
            print('[coap _verify_time_of_last_dict_use function]',e)
            self.my_logger.critical('[coap _verify_time_of_last_dict_use function]' + str(e))
